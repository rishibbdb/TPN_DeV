...
from likelihood_spe_biweight_conv import get_llh_for_iminuit_migrad, get_neg_c_triple_gamma_llh
...
obj_fn = get_neg_c_triple_gamma_llh(eval_network_doms_and_track)

def lets_profile(track_dir, obj_fn, vertex_seed, time, data):
...
for i, tdir in enumerate(init_dirs):
    logls[i] = lets_profile(tdir, obj_fn,
            centered_track_pos,
            centered_track_time,
            fitting_event_data)
