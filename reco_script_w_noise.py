import sys, os
sys.path.insert(0, "/data/user/jyson9/Rishi/TPN_DeV_test/TPN_DeV")
...
from likelihoods.likelihood_conv_mpe_w_noise_logsumexp_gupta import get_neg_c_triple_gamma_llh_SRT_noise
...
network_path = '/data/user/jyson9/Rishi/TPN_DeV_test/TPN_DeV/data/gupta/n96_4comp_w_penalty_1.e-4/new_model_no_penalties_tree_start_epoch_1000.eqx'
eval_network_v = get_network_eval_v_fn(bpath=network_path, dtype=dtype, n_hidden=n_hidden)
eval_network_doms_and_track = get_eval_network_doms_and_track(eval_network_v, dtype=dtype, gupta=gupta, n_comp=n_comp)

PATH_TO_INPUT = '/data/user/jyson9/Rishi/TPN_DeV_test/TPN_DeV/extract_data_from_i3files/test/'
META_FILE_NAME = 'meta_ds_22645_from_0_to_10_10_to_100TeV.ftr'
PULSES_FILE_NAME = 'pulses_ds_22645_from_0_to_10_10_to_100TeV.ftr'
...
geo_file = '/data/user/jyson9/Rishi/TPN_DeV_test/TPN_DeV/data/icecube/detector_geometry.csv'
...
neg_llh = get_neg_c_triple_gamma_llh_SRT_noise(eval_network_doms_and_track)
...
for i in range(0, 1):
    ...
    plot_event(event_data, EVENT_INDEX, geo=geo, outfile=f'event-plots_SRT/Event_plot{EVENT_INDEX}.png')
    ...
    # neg_llh = negative log likelihood for each DOM for each track from data
    # Calculated using PDF driven by given network
    # should this part be in for loop?
    #GAUS_CONV_WIDTH = 3.0
    #neg_llh = get_neg_c_triple_gamma_llh(eval_network_doms_and_track, sigma=GAUS_CONV_WIDTH)
    #neg_llh = get_neg_c_triple_gamma_llh_SRT_noise(eval_network_doms_and_track)
    
    #print("neg_llh = ", neg_llh)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"reco-plots_SRT/Event_plot{EVENT_INDEX}_reco.png", dpi=300)
