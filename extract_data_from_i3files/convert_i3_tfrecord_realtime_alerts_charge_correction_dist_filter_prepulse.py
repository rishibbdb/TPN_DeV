...
parser.add_argument("-id", "--indir", type=str,
                  default="/home/storage2/hans/i3files/alerts/bfrv2/i3_w_millipede_matrix/",
                  dest="INDIR",
                  help="directory containing the .i3 files")

parser.add_argument("-ib", "--infile_base", type=str,
                  default="event_48448_N100_merged_w_energy_loss_preds",
                  dest="INFILE_BASE",
...
parser.add_argument("-o", "--outdir", type=str,
                  default="/home/storage2/hans/i3files/alerts/bfrv2/filter_prepulse/charge_corr_dist/",
                  dest="OUTDIR",
...
for infile in infiles:
    ...
    pulse_data = {'event_id': [], 'sensor_id': [], 'time': [], 'charge': [], 'is_HLC':[], f'corrected_charge_{sigma:.1f}': []}
...
write_path = os.path.join(outdir, f"{infile_base}_from_{file_index_start}_to_{file_index_end}_1st_pulse_sigma_{sigma}.tfrecord")
with tf.io.TFRecordWriter(write_path, options) as writer:
    ...
    # Get dom locations, first hit times, and total charges (for each dom).
        event_data_no_early = sim_handler.get_per_dom_summary_from_sim_data(meta,
                                                            pulses)
        sim_handler.replace_early_pulse(event_data_no_early, pulses)

        event_data = sim_handler.get_per_dom_summary_from_sim_data(meta,
                                                            pulses,
                                                            charge_key=f'corrected_charge_{sigma:.1f}')

        event_data['time'] = event_data_no_early['time']
