...
parser.add_argument("-o", "--outdir", type=str,
                  default="/home/storage2/hans/i3files/lowE/ftr/",
                  dest="OUTDIR",
...
for file_index in range(file_index_start, file_index_end):
    infile = os.path.join(indir, f"{infile_base}_N100_Part0{file_index:.0f}{infile_suffix}")
    print(infile)
...
ofile = os.path.join(outdir, f"pulses_ds_{infile_base}_from_{file_index_start}_to_{file_index_end}_1st_pulse.ftr")
df_pulses.reset_index(drop=True).to_feather(ofile, compression='zstd')
ofile = os.path.join(outdir, f"meta_ds_{infile_base}_from_{file_index_start}_to_{file_index_end}_1st_pulse.ftr")
